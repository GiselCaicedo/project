...
import {
  ...
  listInvoiceObservationsByInvoice,
  createInvoiceObservation,
  updateInvoiceObservation,
  deleteInvoiceObservation,
  listInvoiceCommentsByInvoice,
...
export async function listInvoiceObservationsCtrl(req: Request, res: Response) {
  const { id } = req.params
  if (!id) {
    return res.status(400).json({ success: false, message: 'Falta el identificador de la factura' })
  }
...
export async function createInvoiceObservationCtrl(req: Request, res: Response) {
  const { id } = req.params
  ...
  try {
    const observation = await createInvoiceObservation({ invoiceId: id, content: content.trim(), userId: (req as any)?.user?.id ?? null })
...
export async function updateInvoiceObservationCtrl(req: Request, res: Response) {
...
export async function deleteInvoiceObservationCtrl(req: Request, res: Response) {
  const { observationId } = req.params as { observationId?: string }
  if (!observationId) {
...
// ======================
export async function listInvoiceCommentsCtrl(req: Request, res: Response) {
  const { id } = req.params
  if (!id) {
    return res.status(400).json({ success: false, message: 'Falta el identificador de la factura' })
  }
...
// ======================
export async function getPendingInvoicesByClientCtrl(req: Request, res: Response) {
