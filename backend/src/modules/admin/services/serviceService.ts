...
export type ServiceRecord = {
  ...
  clientsCount: number
  assignments?: Array<{ clientId: string; expiry: string | null }>
}
...
const mapServiceRecord = (service: any): ServiceRecord => ({
  ...
  assignments: Array.isArray((service as any)?.client_service) ? (service as any).client_service.map((a: any) => ({ clientId: a.client_id ?? '', expiry: a.expiry ? (a.expiry instanceof Date ? a.expiry.toISOString() : new Date(a.expiry).toISOString()) : null })) : undefined,
})
...
export async function listServices(): Promise<ServiceRecord[]> {
  const services = await prisma.service.findMany({ distinct: ['name'], include: {
      service_category: true,
      tax_service_tax_one_idTotax: true,
      tax_service_tax_two_idTotax: true,
      _count: { select: { client_service: true } },
    },
    orderBy: [{ updated: 'desc' }, { created: 'desc' }, { name: 'asc' }],
  })
  console.log('listServices fetched', services)
...
export async function fetchServiceCategories(): Promise<ServiceCategoryRecord[]> {
  ...
  return categories.map((category) => ({
    id: category.id,
    name: normalizeText(category.name ?? null, 'Categorí­a sin nombre'),
  }))
}
