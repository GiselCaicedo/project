 ...
 */
export async function getFinancialSummaryReportCtrl(req: Request, res: Response) {
  try {
 ...
 */
export async function getInvoicesReportCtrl(req: Request, res: Response) {
  try {
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) {
      return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
    ...
    const report = await getInvoicesReport(clientId, startDate as string, endDate as string)
 ...
 */
export async function getPaymentsReportCtrl(req: Request, res: Response) {
  try {
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) {
      return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
    ...
    const { startDate, endDate } = req.query
    const report = await getPaymentsReport(clientId, startDate as string, endDate as string)
    return res.json({ success: true, data: report })
  } catch (error) {
 ...
 */
export async function getServicesStatusReportCtrl(req: Request, res: Response) {
  try {
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) {
      return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
 ...
 */
export async function getAccountStatementReportCtrl(req: Request, res: Response) {
  try {
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) {
      return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
    ...
    const { startDate, endDate } = req.query
    const statement = await getAccountStatement(clientId, startDate as string, endDate as string)
