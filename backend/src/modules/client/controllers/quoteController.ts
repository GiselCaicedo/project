...
import { QuoteValidationError, QuoteNotFoundError, type QuoteServiceItemInput } from '../../admin/services/quoteService.js'
...
export async function createCommentCtrl(req: Request, res: Response) {
  const { id } = req.params
  if (!id) {
    return res.status(400).json({ success: false, message: 'Falta el identificador de la cotización' })
  }
...
export async function updateCommentCtrl(req: Request, res: Response) {
  ...
  try {
    ...
    const payload: UpdateCommentInput = { content: content.trim() }
...
export async function listCommentsByQuoteCtrl(req: Request, res: Response) {
  const { id } = req.params
  if (!id) {
    return res.status(400).json({ success: false, message: 'Falta el identificador de la cotización' })
  }
...
export async function approveQuoteCtrl(req: Request, res: Response) {
  const { id } = req.params
  if (!id) {
    return res.status(400).json({ success: false, message: 'Falta el identificador de la cotización' })
  }
