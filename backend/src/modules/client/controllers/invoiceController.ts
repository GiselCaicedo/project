...
import {
  ...
  deleteClientInvoice,
  generateClientInvoiceArtifact,
  listClientInvoiceObservations,
  listClientInvoiceComments,
  createClientInvoiceComment,
  updateClientInvoiceComment,
  deleteClientInvoiceComment,
...
export async function deleteClientInvoiceCtrl(req: Request, res: Response) {
  try {
    const { id } = req.params
    if (!id) return res.status(400).json({ success: false, message: 'Falta el identificador de la factura' })
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
...
export async function downloadClientInvoiceArtifactCtrl(req: Request, res: Response) {
  try {
    const { id, format } = req.params as { id?: string; format?: string }
    if (!id) return res.status(400).json({ success: false, message: 'Falta factura' })
    const allowed = ['pdf', 'xml', 'zip']
    const normalized = (format ?? '').toLowerCase()
    if (!allowed.includes(normalized)) return res.status(400).json({ success: false, message: 'Formato inválido' })

    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
    const artifact = await generateClientInvoiceArtifact(clientId, id, normalized as any)
...
export async function listClientInvoiceObservationsCtrl(req: Request, res: Response) {
  try {
    const { id } = req.params
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
...
export async function listClientInvoiceCommentsCtrl(req: Request, res: Response) {
  try {
    const { id } = req.params
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
...
export async function createClientInvoiceCommentCtrl(req: Request, res: Response) {
  try {
    const { id } = req.params
...
export async function updateClientInvoiceCommentCtrl(req: Request, res: Response) {
  try {
    ...
    if (!updated) return res.status(404).json({ success: false, message: 'Comentario no encontrado' })
...
export async function deleteClientInvoiceCommentCtrl(req: Request, res: Response) {
  try {
    const { commentId } = req.params as { commentId?: string }
    if (!commentId) return res.status(400).json({ success: false, message: 'Falta identificador del comentario' })
    const clientId = (req as any)?.user?.empresaid as string | undefined
    if (!clientId) return res.status(400).json({ success: false, message: 'Falta empresa en el token' })
    const ok = await deleteClientInvoiceComment(clientId, commentId)
    if (!ok) return res.status(404).json({ success: false, message: 'Comentario no encontrado' })
    return res.status(204).send()
  } catch (error) {
...
export async function reportClientInvoicePaymentCtrl(req: Request, res: Response) {
  try {
    const { id } = req.params
    const clientId = (req as any)?.user?.empresaid as string | undefined
    ...
    const file: IncomingFilePayload | null = typeof req.body?.documentFile === 'object' ? {
    ...
    if (!file || !file.name || !file.data) return res.status(400).json({ success: false, message: 'Falta comprobante' })
