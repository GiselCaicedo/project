...
import type { InvoiceArtifact, InvoiceArtifactFormat } from '../../admin/services/invoiceService.js'
...
export async function deleteClientInvoice(clientId: string, invoiceId: string): Promise<boolean> {
...
export async function generateClientInvoiceArtifact(
  clientId: string,
  invoiceId: string,
...
export type ClientInvoiceObservation = { id: string; invoiceId: string; userId: string | null; content: string; created: string | null; updated: string | null; status: boolean | null }
export async function listClientInvoiceObservations(clientId: string, invoiceId: string): Promise<ClientInvoiceObservation[]> {
  const invoice = await prisma.invoice.findFirst({ where: { id: invoiceId, client_id: clientId } })
  if (!invoice) return []
...
export type ClientInvoiceComment = { id: string; invoiceId: string; userId: string | null; content: string; created: string | null; updated: string | null; status: boolean | null }
export async function listClientInvoiceComments(clientId: string, invoiceId: string): Promise<ClientInvoiceComment[]> {
  const invoice = await prisma.invoice.findFirst({ where: { id: invoiceId, client_id: clientId } })
  if (!invoice) return []
...
export async function createClientInvoiceComment(
  clientId: string,
  payload: { invoiceId: string; userId?: string | null; content: string },
): Promise<ClientInvoiceComment> {
  const invoice = await prisma.invoice.findFirst({ where: { id: payload.invoiceId, client_id: clientId } })
  if (!invoice) throw new Error('Factura no pertenece al cliente')
  const created = await prisma.invoice_comment.create({ data: { id: randomUUID(), invoice_id: payload.invoiceId, user_id: payload.userId ?? null, content: payload.content, status: true } as any })
  const toIso = (d: any) => (d instanceof Date ? d.toISOString() : d ? new Date(d).toISOString() : null)
...
export async function updateClientInvoiceComment(
  clientId: string,
  commentId: string,
  payload: { content: string },
): Promise<ClientInvoiceComment | null> {
...
export async function deleteClientInvoiceComment(clientId: string, commentId: string): Promise<boolean> {
  const existing = await prisma.invoice_comment.findUnique({ where: { id: commentId } })
...
export async function attachClientPaymentProof(clientId: string, invoiceId: string, url: string) {
  const invoice = await prisma.invoice.findFirst({ where: { id: invoiceId, client_id: clientId } })
  if (!invoice) throw new Error('Factura no pertenece al cliente')
