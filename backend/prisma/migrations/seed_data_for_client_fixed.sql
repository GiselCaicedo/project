-- Script mejorado para crear datos de ejemplo para el cliente (IDEMPOTENTE)
-- Client ID: 05821cfe-4c68-42f0-93b5-c99d56d548e1

-- ============================================
-- LIMPIEZA PREVIA (solo para este cliente)
-- ============================================
...
BEGIN
  ...
  DELETE FROM "payment_attachment" WHERE invoice_id IN (SELECT id FROM "invoice" WHERE client_id = target_client_id);
  DELETE FROM "invoice_comment" WHERE invoice_id IN (SELECT id FROM "invoice" WHERE client_id = target_client_id);
  DELETE FROM "invoice_observation" WHERE invoice_id IN (SELECT id FROM "invoice" WHERE client_id = target_client_id);
  DELETE FROM "invoice_detail" WHERE invoice_id IN (SELECT id FROM "invoice" WHERE client_id = target_client_id);

  DELETE FROM "quote_comment" WHERE quote_id IN (SELECT id FROM "quote" WHERE client_id = target_client_id);
  DELETE FROM "quote_observation" WHERE quote_id IN (SELECT id FROM "quote" WHERE client_id = target_client_id);
  DELETE FROM "quote_attachment" WHERE quote_id IN (SELECT id FROM "quote" WHERE client_id = target_client_id);
  DELETE FROM "quote_detail" WHERE quote_id IN (SELECT id FROM "quote" WHERE client_id = target_client_id);

  DELETE FROM "client_service" WHERE client_id = target_client_id;
  DELETE FROM "invoice" WHERE client_id = target_client_id;
  DELETE FROM "quote" WHERE client_id = target_client_id;

  RAISE NOTICE 'Datos anteriores del cliente eliminados';
END $$;
...
BEGIN
  ...
  subtotal_q1 := 20000000.00 + 12000000.00;
  subtotal_q2 := 35000000.00;
  subtotal_q3 := 350000.00 * 20 + 6000000.00;
  ...
  INSERT INTO "quote_detail" (id, quote_id, service_id, item, quantity, total_value, created, updated, status)
  VALUES
    (gen_random_uuid(), quote_id_3, srv_consulting, 1, 20, 7000000.00, NOW(), NOW(), true),
...
BEGIN
  SELECT q.id INTO quote_id_1 FROM "quote" q WHERE q.client_id = target_client_id AND q.consecutive = 'COT-001' LIMIT 1;
  SELECT q.id INTO quote_id_3 FROM "quote" q WHERE q.client_id = target_client_id AND q.consecutive = 'COT-003' LIMIT 1;
...
-- ============================================
-- PASO 7: Crear Servicios activos para el cliente
-- ============================================
...
BEGIN
  SELECT i.id INTO invoice_id_1
  FROM "invoice" i
  WHERE i.client_id = '05821cfe-4c68-42f0-93b5-c99d56d548e1'
    AND i.consecutive = 'FAC-001'
  LIMIT 1;
...
BEGIN
  ...
  SELECT id INTO quote_id_1 FROM "quote" WHERE client_id = target_client_id AND consecutive = 'COT-001' LIMIT 1;
  SELECT id INTO quote_id_2 FROM "quote" WHERE client_id = target_client_id AND consecutive = 'COT-002' LIMIT 1;
  SELECT i.id INTO invoice_id_1 FROM "invoice" i WHERE i.client_id = target_client_id AND i.consecutive = 'FAC-001' LIMIT 1;
  SELECT i.id INTO invoice_id_2 FROM "invoice" i WHERE i.client_id = target_client_id AND i.consecutive = 'FAC-002' LIMIT 1;
...
SELECT '=== DATOS CREADOS CORRECTAMENTE ===' as info;

SELECT 'Cotizaciones:' as tipo, COUNT(*) as cantidad FROM "quote" WHERE client_id = '05821cfe-4c68-42f0-93b5-c99d56d548e1';
SELECT 'Facturas:' as tipo, COUNT(*) FROM "invoice" WHERE client_id = '05821cfe-4c68-42f0-93b5-c99d56d548e1';
