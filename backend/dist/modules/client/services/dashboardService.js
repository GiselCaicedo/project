 ...
 */
export async function fetchClientBalance(clientId) {
 ...
 */
export async function fetchServiceConsumption(clientId) {
    ...
    const serviceConsumptionData = await Promise.all(clientServices.map(async (cs) => {
            ...
            try {
                ...
                if (parsed && typeof parsed === 'object') {
                    ...
                    if (typeof value === 'number')
                        ...
                        consumption = parseCurrency(value);
                    else
                        consumption = 0;
                }
                else {
 ...
 */
export async function fetchClientExpirations(clientId, daysAhead = 30) {
    const now = new Date();
