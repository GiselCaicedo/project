...
// =================== General settings ===================
export async function getActiveGeneralSetting(_) {
...
// =================== SMTP ===================
export async function getActiveSmtpConfig(_) {
...
// =================== Security policy ===================
export async function getSecurityPolicy(_) {
...
export async function saveSecurityPolicy(payload) {
    const data = {
        ...
        max_concurrent_sessions: typeof payload.max_concurrent_sessions === 'number' ? payload.max_concurrent_sessions : null,
        is_active: true,
        status: true,
    };
    return prisma.$transaction(async (tx) => {
...
// =================== Session policy ===================
export async function getSessionPolicy(_) {
...
export async function saveSessionPolicy(payload) {
    const data = {
        ...
        absolute_session_minutes: typeof payload.absolute_session_minutes === 'number' ? payload.absolute_session_minutes : null,
        remember_me_days: typeof payload.remember_me_days === 'number' ? payload.remember_me_days : null,
        lock_after_failed_attempts: typeof payload.lock_after_failed_attempts === 'number' ? payload.lock_after_failed_attempts : null,
...
// =================== Password policy ===================
export async function getPasswordPolicy(_) {
...
export async function savePasswordPolicy(payload) {
    const data = {
        ...
        disallow_common_passwords: typeof payload.disallow_common_passwords === 'boolean' ? payload.disallow_common_passwords : true,
...
// =================== Company profile ===================
export async function getCompanyProfile(clientId) {
    return prisma.client.findUnique({
        where: { id: clientId },
        select: { id: true, name: true, status: true, created: true, updated: true },
    });
}
export async function updateCompanyProfile(clientId, payload) {
    const data = {};
    if (typeof payload.name !== 'undefined')
        data.name = payload.name;
    if (typeof payload.status !== 'undefined')
