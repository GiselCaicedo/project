...
const roundCurrencyValue = (value) => Number.isFinite(value) ? Number.parseFloat(value.toFixed(2)) : 0;
...
export async function updateQuoteServices(id, services) {
    ...
    const hasInvoice = (quote.quote_attachment ?? []).some((a) => Boolean(a.invoice_id));
    if (hasInvoice) {
...
const createInvoiceForQuote = async (tx, quote, now) => {
    ...
    const details = (quote.quote_detail ?? []).map((detail, index) => {
        const quantity = typeof detail.quantity === 'number' && Number.isFinite(detail.quantity) ? detail.quantity : 1;
...
const resolveQuoteServiceDetails = async (services) => {
    ...
    const normalized = services.map((service) => ({
        ...
        unitPrice: typeof service.unitPrice === 'number' && Number.isFinite(service.unitPrice) ? service.unitPrice : null,
        total: typeof service.total === 'number' && Number.isFinite(service.total) ? service.total : null,
    }));
    if (normalized.some((service) => service.serviceId.length === 0 || !Number.isFinite(service.quantity) || service.quantity <= 0)) {
...
export async function listQuotes() {
    ...
    return records.map((quote) => {
        ...
        const invoiceSummary = firstAttachment
            ? {
                id: firstAttachment.invoice_id ?? null,
                number: firstAttachment.invoice
                    ? normalizeText(firstAttachment.invoice.consecutive ?? firstAttachment.invoice.description ?? firstAttachment.invoice.id, firstAttachment.invoice.id)
        ...
        return {
            id: quote.id,
            reference: normalizeText(quote.consecutive ?? quote.description, quote.id),
...
export async function getQuoteById(id) {
    ...
    const observations = (quote.quote_observation ?? []).map((obs) => ({
        id: obs.id,
    ...
    const comments = (quote.quote_comment ?? []).map((comment) => ({
        id: comment.id,
...
export async function updateObservation(observationId, payload) {
...
export async function listObservationsByQuote(quoteId) {
