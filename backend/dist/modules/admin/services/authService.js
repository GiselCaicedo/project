...
function inferCategoryFromPermissions(perms) {
    if (perms.some(p => p.startsWith('admin_') || p === 'panel_admin'))
        return 'ADMIN';
    if (perms.some(p => p.startsWith('client_') || p === 'panel_client'))
        return 'CLIENT';
    return 'GENERAL';
}
export const loginUser = async (identifier, password) => {
    ...
    const roleCategory = user.role?.panel ??
