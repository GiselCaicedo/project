import { createPayment, deletePayment, fetchPaymentById, fetchPaymentsList, updatePayment, } from '../services/paymentService.js';
import { saveBase64File } from '../../../utils/fileStorage.js';
const parseAttachments = async (value) => {
    if (!Array.isArray(value))
        return [];
    const processed = await Promise.all(value.map(async (entry) => {
        ...
        if (isFile && url.startsWith('data:')) {
            try {
                ...
                const savedUrl = await saveBase64File({ name: fileName, type: fileType, data: url }, { folder: 'payments' });
                url = savedUrl;
            }
            catch (error) {
    ...
    return processed.filter((attachment) => attachment !== null && attachment.url.length > 0);
};
...
export async function listPaymentsCtrl(_req, res) {
    const data = await fetchPaymentsList();
    res.json({ success: true, data });
}
export async function getPaymentByIdCtrl(req, res) {
    const { id } = req.params;
    if (!id) {
...
export async function updatePaymentCtrl(req, res) {
    const { id } = req.params;
    if (!id) {
        return res.status(400).json({ success: false, message: 'Falta el identificador del pago' });
    }
...
export async function deletePaymentCtrl(req, res) {
    const { id } = req.params;
    if (!id) {
        return res.status(400).json({ success: false, message: 'Falta el identificador del pago' });
    }
