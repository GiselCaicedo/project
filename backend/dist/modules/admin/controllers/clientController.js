import { assignServiceToClient, createClient, deleteClient, fetchClientById, fetchClientsWithDetails, updateClient, updateClientServiceAssignment, } from '../services/clientService.js';
const CLIENT_STATUSES = ['active', 'inactive', 'onboarding'];
const CLIENT_TYPES = ['natural', 'juridica'];
const isClientStatus = (value) => typeof value === 'string' && CLIENT_STATUSES.includes(value);
const isClientType = (value) => typeof value === 'string' && CLIENT_TYPES.includes(value);
const parseClientDetails = (value) => {
...
const buildServiceAssignmentPayload = (body) => {
    ...
    const payload = {
        ...
        observations: nullable(body?.observations),
    };
    return { data: payload };
};
export async function listClientsCtrl(_req, res) {
    const payload = await fetchClientsWithDetails();
    res.json({ success: true, data: payload });
}
export async function getClientByIdCtrl(req, res) {
    const { id } = req.params;
    if (!id) {
...
export async function createClientCtrl(req, res) {
...
export async function assignClientServiceCtrl(req, res) {
    const { id } = req.params;
    if (!id) {
        return res.status(400).json({ success: false, message: 'Falta el identificador del cliente' });
    }
