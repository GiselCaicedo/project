            ...
            <div className="space-y-6">
              ...
              {service.observations && service.observations.length > 0 && (
                <section className="rounded-xl border border-amber-200 bg-amber-50 p-6 shadow-sm">
                  <h2 className="text-base font-semibold text-amber-900">Observaciones del Administrador</h2>
                  <div className="mt-4 space-y-3">
                    {service.observations.map((obs) => (
                      <div key={obs.id} className="rounded-lg border border-amber-300 bg-white p-4">
                        <p className="text-sm text-gray-800 whitespace-pre-line">{obs.content}</p>
                        {obs.created && (
