...
export default function MainReportsClient({
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader breadcrumbs={breadcrumbs} title="Reportes" description="Reportes y métricas de tu cuenta." />

      {errorMessages.length > 0 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
          <p className="text-sm font-medium text-amber-900">No fue posible cargar toda la informaciÃ³n:</p>
    ...
    <section className="space-y-4">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3 text-xs text-gray-500">
          ...
          <span>
            Impuesto 1:
            {formatCurrency(totals.taxOne)}
          </span>
          <span>
            Impuesto 2:
            {formatCurrency(totals.taxTwo)}
          </span>
          <span>
            Total:
            ...
            <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                  <tr>
                    <th className="px-3 py-2 text-left font-medium">DescripciÃ³n</th>
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {invoices.map(invoice => (
                    <tr key={invoice.id}>
                      <td className="px-3 py-2 text-gray-700">{invoice.description ?? 'Factura sin descripciÃ³n'}</td>
        ...
        ? (
            <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                  <tr>
                    <th className="px-3 py-2 text-left font-medium">CÃ³digo</th>
                    <th className="px-3 py-2 text-left font-medium">MÃ©todo</th>
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <ServiceStatusList title="Por vencer" items={services.expiringSoon} emptyMessage="Sin servicios prÃ³ximos a vencer." />
            ...
            <ul className="mt-3 space-y-3">
              {items.map(item => (
                <li key={item.id} className="text-sm text-gray-700">
                  ...
                  <p className="text-xs text-gray-500">
                    {item.service?.frequency ? `${item.service.frequency} Â· ` : ''}
                    Precio:
