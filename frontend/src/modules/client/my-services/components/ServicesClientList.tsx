...
export default function ServicesClientList({ services }: Props) {
  ...
  const columns: ColDef<ClientServiceSummary>[] = useMemo(() => {
    ...
    return [
      ...
      { headerName: 'Precio', field: 'price', valueFormatter: p => p.value == null ? '-' : new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(p.value), minWidth: 120, cellClass: 'text-right justify-end' },
      ...
      { headerName: 'Estado', field: 'status', minWidth: 110, cellClass: 'text-center justify-center' },
      {
        ...
        minWidth: 200,
        cellRenderer: (params) => {
          const observations = params.data?.observations;
          if (!observations || observations.length === 0) {
            return <span className="text-gray-400 text-xs italic">Sin observaciones</span>;
          }
          const latestObs = observations[observations.length - 1];
          return (
            <div className="text-sm text-gray-700">
              <p className="line-clamp-2">{latestObs.content}</p>
            </div>
          );
        },
      },
