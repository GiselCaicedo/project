...
import { ArrowUpRight, FileText, Loader2, Mail, Pencil, RotateCcw } from 'lucide-react';
...
const attachmentTone: Record<string, { label: string; tone: string }> = {
  ...
  pendiente: { label: 'Pendiente', tone: 'bg-amber-100 text-amber-700' },
  rechazada: { label: 'Rechazada', tone: 'bg-red-100 text-red-700' },
  en_proceso: { label: 'En proceso', tone: 'bg-primary-100 text-primary-700' },
};
...
export default function QuoteDetailInspector({
  ...
  const renderServices = () => {
    ...
    if (quote.services.length === 0) {
      return (
        <div className="rounded-2xl  border-dashed border-gray-200 bg-gray-50 px-4 py-5 text-sm text-gray-500">
                        ...
                        <form onSubmit={handleSendEmail} className="space-y-3">
                          <div>
                            ...
                            <textarea
                              ...
                              rows={2}
                              className="w-full rounded-2xl border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 focus:outline-none"
                            />
                          </div>
                          <div>
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-2 rounded-full border border-primary-200 bg-primary-500 px-4 py-2 text-xs font-semibold text-white transition hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            {actionLoading === 'email'
                              ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
