...
export default async function Dashboard(props: {
  ...
  const payload = token ? await verifyToken<any>(token) : null;
  const userName = payload?.name || payload?.user || payload?.usuario || '';
  const avatarUrl = payload?.picture || payload?.avatar || null;
