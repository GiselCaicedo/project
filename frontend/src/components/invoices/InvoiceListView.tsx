...
export default function InvoiceListView({
  ...
  const handleDelete = async (id: string) => {
    const confirmation = window.confirm('Â¿Deseas eliminar esta factura? Esta acciÃ³n no se puede deshacer.');
    if (!confirmation) {
      return;
    }
    try {
  ...
  return (
    <div className="space-y-6" ref={containerRef}>
      <PageHeader
        ...
        title="Facturas"
        description="Gestiona facturas, descargas y envÃ­os a clientes desde un Ãºnico lugar."
      ...
      >
        <div className="px-5 py-6">
          {panel.mode === 'edit' && loadingInvoice
            ? (
                <p className="text-sm text-gray-500">Cargando facturaâ€¦</p>
              )
            : (
                <InvoiceForm
                  catalog={catalog}
