...
export default function ClientListView({ initialClients, initialParameters, initialError = null }: ClientListViewProps) {
  ...
  const columns = useMemo<ColDef<ClientRow>[]>(() => {
    const baseColumns: ColDef<ClientRow>[] = [
      {
        ...
        cellRenderer: (params: ICellRendererParams<ClientRow>) => (
          <button
            type="button"
            onClick={() => params.data && navigateToDetail(params.data.id)}
      ...
      },
      {
        ...
        minWidth: 140,
        cellRenderer: (params: ICellRendererParams<ClientRow>) => {
          const status = params.data?.status as ClientStatus | undefined;
          const tone = status ? statusLabel[status] : undefined;
          if (!tone) {
            return (
    ...
    const dynamicColumns = parameters.map(parameter => ({
      headerName: parameter.name,
      field: parameter.id,
      minWidth: 160,
      flex: 1,
      valueGetter: (params: any) => params.data?.parameterValues?.[parameter.id] ?? '—',
      tooltipValueGetter: (params: any) => params.data?.parameterValues?.[parameter.id] ?? '',
    })) as unknown as ColDef<ClientRow>[];

    const actionsColumn: ColDef<ClientRow> = {
      headerName: 'Acciones',
      // no bound field for actions column
      minWidth: 160,
      maxWidth: 180,
      pinned: 'right',
      sortable: false,
      cellRenderer: (params: ICellRendererParams<ClientRow>) => (
        <div className="flex items-center gap-2">
          ...
          >
            <Eye className="h-3.5 w-3.5" />{' '}{params.data?.name ?? 'Ver'}
          </button>
          <button
            type="button"
            onClick={() => params.data && setClientToDelete(params.data)}
